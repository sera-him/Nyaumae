...
## 语义检索与云端同步（2026-09-17 新增）

- 语义检索：在 AI 设置的高级设置中开启后，知识库检索在本地全文检索之上叠加 BYOK 向量检索（OpenAI 兼容 `/embeddings`，复用对话服务的 API Key）。知识库向量按「服务地址 + 模型 + 语料指纹」缓存于浏览器 IndexedDB；嵌入服务不可用时自动回退为纯关键词检索，不会中断对话。
- 云端同步：在 AI 设置的「云端同步」中配置同步地址和同步密钥后，对话状态与 AI 配置（不含 API Key）经浏览器端 AES-GCM 加密后上传至同步 Worker（`cloud/sync-worker/`，Cloudflare Workers + KV），多台设备凭同一同步密钥共享数据。合并策略为整包最后写入获胜（LWW）。评论、点赞、阅读进度经同一适配器上云。
- 同步密钥只保存在当前浏览器；服务端按 SHA-256(密钥) 分命名空间且只存密文，忘记密钥无法找回云端数据。

## 当前限制

- 不同服务商对 OpenAI 兼容流式格式的兼容程度可能不同，连接测试应在实际模型上执行。
- 站内资料的正史状态来自现有数据；模型推断不会自动写入正式正史。
- 云端同步为整包 LWW，不适合多设备同时编辑同一对话的场景。
