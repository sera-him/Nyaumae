...
function nearestCoreDistance(id: number, adjacency: number[][]) {
  const distances = graphDistances(id, adjacency);
  return Math.min(...[...CORE_IDS].map((core) => distances[core]));
}
...
function chooseSpawns(nodes: NeuralNode[], edges: Synapse[], adjacency: number[][]) {
  ...
  for (const node of nodes) {
    ...
    if (graphDistances(node.id, adjacency)[otherId] !== 6) continue;
    if (enhancedDegree[node.id] !== enhancedDegree[otherId] || Math.abs(degree[node.id] - degree[otherId]) > 1) continue;
    if (nearestCoreDistance(node.id, adjacency) !== nearestCoreDistance(otherId, adjacency)) continue;
    const a = localMetrics(node.id, nodes, edges, adjacency);
    const b = localMetrics(otherId, nodes, edges, adjacency);
    ...
    if (!best || score < best.score) best = { a: node.id, b: otherId, score };
  }
  ...
  for (const node of nodes) {
    ...
    if (node.id >= otherId || node.core || nodes[otherId].core) continue;
    if (adjacentToCore[node.id] || adjacentToCore[otherId]) continue;
    const d = graphDistances(node.id, adjacency)[otherId];
...
export function generateMap(seed = Math.floor(Math.random() * 900000) + 100000): NeuralMap {
  ...
  // The strengthened subgraph is a toroidal lattice: connected and bridge-free.
  for (let row = 0; row < 10; row += 1) {
    for (let col = 0; col < 10; col += 1) {
      const id = row * 10 + col;
      addEdge(id, row * 10 + ((col + 1) % 10), true);
      addEdge(id, ((row + 1) % 10) * 10 + col, true);
    }
  }
  ...
  for (const core of corePairs) {
    ...
    const candidates = nodes
      .filter((node) => !node.core && !lookup.has(edgeKey(core, node.id)))
  ...
  for (let a = 0; a < 100; a += 1) {
    ...
    for (let b = a + 1; b < 100; b += 1) {
      if (nodes[b].core || lookup.has(edgeKey(a, b))) continue;
...
export function applyAction(state: GameState, source: number, target?: number): GameState {
  ...
  if (state.mode === 'expand' && state.ap[side] >= 1) {
    ...
    return {
      ...state, map, ap: { ...state.ap, [side]: state.ap[side] - 1 },
      actionTaken: { ...state.actionTaken, [side]: true }, selected: target, mode: 'inspect',
      log: appendLog(state, `${label}沿强化突触扩张至 ${target + 1} 号节点。`),
    };
  }
