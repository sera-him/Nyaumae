...
export default function Navigation({ onSearchClick }: NavigationProps) {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div className="aurora-menu-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={prefersReducedMotion ? { duration: 0 } : { duration: .18 }} onClick={(event) => { if (event.target === event.currentTarget) closeMenu(); }}>
            <motion.div id="aurora-navigation-panel" ref={menuPanelRef} role="dialog" aria-modal="true" aria-label="全站目录" className="aurora-menu-shell" initial={{ opacity: 0, y: -18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={prefersReducedMotion ? { duration: 0 } : { duration: .28, ease: [0.22, 1, 0.36, 1] }}>
