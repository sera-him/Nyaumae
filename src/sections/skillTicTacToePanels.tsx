  ...
  return (
    <div className="space-y-6">
      <div className="bg-nc-bg-secondary border border-nc-violet/10 rounded-xl p-5">
        <h3 className="text-lg font-bold text-nc-text mb-3">{L("核心机制")}</h3>
        <div className="space-y-2 text-sm text-nc-text-secondary">
          <p>{L("• 每个格子有独立的成功概率，落子时掷概率骰判定是否成功")}</p>
          <p>{L("• 失败跳过回合，但根据格子位置获得 SP 补偿：中心 +3，角 +4，边 +6")}</p>
          <p>{L("• SP 上限 30，每回合 +3 收入，可用技能/购买职业")}</p>
          <p>{L("• 先连成三连者获胜")}</p>
        </div>
      </div>
...
export function ProfessionsPanel() {
  return (
    <div className="space-y-4">
      <div className="bg-nc-bg-secondary border border-nc-violet/10 rounded-xl p-5">
        <h3 className="text-lg font-bold text-nc-text mb-4">{L("职业系统（8 种）")}</h3>
        ...
        <table className="min-w-[600px] w-full text-sm">
          <thead>
            <tr className="border-b border-nc-violet/10">
              <th className="text-left px-4 py-3 text-nc-text-muted font-medium">{L("技能")}</th>
              <th className="text-left px-4 py-3 text-nc-text-muted font-medium">SP</th>
              <th className="text-left px-4 py-3 text-nc-text-muted font-medium">{L("效果")}</th>
            </tr>
          </thead>
          <tbody>
