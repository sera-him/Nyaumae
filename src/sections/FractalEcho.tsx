...
export default function FractalEcho() {
  ...
  const candidates = useMemo<Candidate[]>(() => {
    if (!tip) return [];
    const offsets = usingEcho ? [-34, 0, 34].map((offset, index) => offset + (lastPattern[player === 'A' ? 'B' : 'A'].includes(index) ? (player === 'A' ? 5 : -5) : 0)) : [-34, 0, 34];
  ...
  const confirmGrowth = () => {
    ...
    candidates.forEach((candidate, index) => {
      const canPrune = round >= 5;
      const target = canPrune
    ...
    if ([16, 32, 48].includes(nextTurn)) {
      setEchoes((stock) => ({ A: Math.min(2, stock.A + 1), B: Math.min(2, stock.B + 1) }));
    }
  ...
  return (
    <section className="fractal-game">
      ...
      <div className="fractal-scoreboard">
        ...
        <div className="fractal-round-card">
          <div><span>ROUND</span><strong>{String(round).padStart(2, '0')}</strong><span>/ 32</span></div>
          <p>{finished ? winner : `${PLAYER_NAME[player]} 行动`}</p>
        </div>
      </div>
      ...
      {showRules && (
        <div className="fractal-rules">
          <p><b>当前阶段 · {phase.name}</b>　{phase.note}</p>
          <p>每次点击一个己方生长点，生成三根计分子枝；第17轮起最多保留一根新生长枝。新枝端点距对手枝条不足6px时剪去该枝及后代。</p>
          <p>每人最多16个有效生长点。回声在第9、17、25轮补充1次，最多储存2次；它会复刻对手最近一次的保留节奏。</p>
        </div>
      )}

      <div className="fractal-board-shell">
        <svg className="fractal-board" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label="分形回响游戏棋盘">
          <defs>
            <filter id="branch-glow"><feGaussianBlur stdDeviation="2.8" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
            <pattern id="fractal-grid" width="40" height="40" patternUnits="userSpaceOnUse"><path d="M 40 0 L 0 0 0 40" fill="none" stroke="rgba(255,255,255,.035)" strokeWidth="1" /></pattern>
          </defs>
          <rect width={WIDTH} height={HEIGHT} fill="url(#fractal-grid)" />
          ...
          {branches.map((branch) => (
            <g key={branch.id}>
              <line
                ...
                stroke={PLAYER_COLOR[branch.owner]}
                strokeOpacity={0.38 + Math.min(0.5, branch.depth * 0.035)}
                strokeWidth={Math.max(1.4, 5.5 - branch.depth * 0.22)}
                strokeLinecap="round"
              />
              {branch.active && (
                <circle
                  cx={branch.x2} cy={branch.y2} r={branch.owner === player && !finished ? 8 : 4.5}
                  fill={PLAYER_COLOR[branch.owner]}
                  className={branch.owner === player && !finished ? 'fractal-tip active-tip' : 'fractal-tip'}
                  onClick={() => selectTip(branch)}
                  role="button"
                  aria-label={`${PLAYER_NAME[branch.owner]}生长点`}
                />
              )}
            </g>
          ))}
        ...
        {finished && (
          <div className="fractal-result">
            <Leaf size={28} />
            <span>32轮生长结束</span>
            <h3>{winner}</h3>
            <p>A {scores.A.toLocaleString()} · {scores.B.toLocaleString()} B</p>
            <button type="button" onClick={reset}><RotateCcw size={15} /> 再生一局</button>
          </div>
        )}
      </div>
      ...
      <footer className="fractal-controls">
        ...
        <div className="fractal-buttons">
          <button type="button" className={`fractal-echo-button ${usingEcho ? 'is-on' : ''}`} onClick={toggleEcho} disabled={finished || echoes[player] <= 0}>
            <Copy size={15} /> 克隆回声 <b>{echoes[player]}</b>
          </button>
          <button type="button" className="fractal-confirm-button" onClick={confirmGrowth} disabled={!tip || finished}>
            <Check size={16} /> 确认生长
          </button>
        </div>
      </footer>

      <div className="fractal-sequence">
        <span>分形节拍</span>
        {Array.from({ length: 32 }, (_, index) => {
          const side = firstPlayer(index + 1);
          return <i key={index} className={`${side === 'A' ? 'a' : 'b'} ${index + 1 === round ? 'now' : ''}`}>{side}</i>;
        })}
      </div>
    </section>
  );
