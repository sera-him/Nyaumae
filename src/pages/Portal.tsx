...
const slidesEn = [
  { src: '/1.jpg', alt: 'Balloons and the Candy House', title: 'Balloons and the Candy House' },
  { src: '/1-generated.png', alt: 'Math World', title: 'Math World' },
  { src: '/2.jpg', alt: 'The Courtyard of Formulas and the Home Page', title: 'The Courtyard of Formulas and the Home Page' },
  { src: '/2-time-nodes.png', alt: 'Five Time Nodes', title: 'Five Time Nodes' },
  { src: '/3.jpg', alt: 'Clouds Drift into Cinnamoroll’s Room', title: 'Clouds Drift into Cinnamoroll’s Room' },
  { src: '/3-generated.png', alt: 'Six World Entrances', title: 'Six World Entrances' },
];

const slideMetadataZh = [
...
const slideMetadataEn = [
  { location: 'Star Hall · Ring Library', time: 'Afternoon', region: 'Universe Overview' },
  { location: 'Deep in the Mind', time: 'Eternal', region: 'Cognitive Space' },
  { location: 'Zhehua Campus', time: 'Afternoon', region: 'The Real World' },
  { location: 'Timeline', time: 'Interwoven', region: 'Chronicle' },
  { location: 'Star Hall · Lounge', time: 'Evening', region: 'Everyday' },
  { location: 'Cosmic Junction', time: 'Dusk', region: 'Entrance' },
];

const commandExamplesZh = ['打开小禾第一次出现的章节', '画出喵呜到游泳池的关系路线', '继续上次 NCTB 测试', '启动 Cat Machine', '检查第二卷第五章是否违反正史'];
const commandExamplesEn = ['Open the chapter where Xiaohe first appears', 'Map the relationship path from Miaowu to the swimming pool', 'Continue the last NCTB test', 'Launch Cat Machine', 'Check whether volume 2 chapter 5 breaks canon'];

// English mirror for the directory cards rendered from NAVIGATION_GROUPS.
const NAVIGATION_GROUP_EN: Record<string, { label: string; caption: string; description: string }> = {
  miia: { label: 'Miia', caption: 'MIIA', description: 'Miia’s world, math notes & poetry' },
  world: { label: 'World', caption: 'WORLD', description: 'Settings, history, organizations & the future line' },
  stories: { label: 'Stories', caption: 'STORIES', description: 'Five story worlds & the full reading directory' },
  characters: { label: 'Characters', caption: 'CHARACTERS', description: 'Character profiles, relationships & faction filters' },
  math: { label: 'Math', caption: 'MATH', description: 'FSIII, FLA & height-weight models' },
  playground: { label: 'Playground', caption: 'PLAYGROUND', description: 'Playable games & Scratch works' },
  ai: { label: 'AI', caption: 'AI CHAT', description: 'Chat spaces, themes & AI settings' },
  other: { label: 'Other', caption: 'OTHER', description: 'Site search, cognition lab & local data' },
};
...
function NeuralCommandConsole() {
  ...
  const examples = getLocale() === 'en' ? commandExamplesEn : commandExamplesZh;
  return <div className="portal-command-console"><div className="portal-command-heading"><span><BrainCircuit size={15} /> AI ASSISTANT / FIND & EXPLORE</span><small>{command.length} / 200</small></div><form onSubmit={(event) => submit(event)}><Search size={18} aria-hidden="true" /><input value={command} onChange={(event) => setCommand(event.target.value)} maxLength={200} placeholder={L("需要时，让 AI 帮你查找内容")} aria-label={L("向站内 AI 助手输入问题")} /><button type="submit" data-motion-ripple="true" aria-label={L("发送给站内 AI 助手")}><ArrowRight size={18} /></button></form><div className="portal-command-actions"><button type="button" onClick={() => submit({ preventDefault: () => undefined } as React.FormEvent<HTMLFormElement>, true)}>{L("搜索全部内容")}</button><span>{L("可以直接搜索，也可以带着问题进入对话")}</span></div><div className="portal-command-examples" aria-label={L("辅助问题示例")}>{examples.slice(0, 3).map((example) => <button type="button" key={example} onClick={() => setCommand(example)}>{example}</button>)}</div></div>;
}
...
export default function Portal() {
  ...
  const slides = _en ? slidesEn : slidesZh;
  const slideMetadata = _en ? slideMetadataEn : slideMetadataZh;
  ...
  const lastViewedDescription = lastViewed
    ? (_en ? `Back to: ${lastViewed.label}` : `回到：${lastViewed.label}`)
    : (_en ? 'No history yet — start with World Lore' : '还没有记录，从世界观开始');
  ...
  return (
    <div className="portal-aurora">
      <section className="portal-hero" data-motion-loop data-motion-kind="ambient">
        ...
        <NeuralParticles />
        <p className="portal-particle-hint">{L("✦ 移动指针唤醒星尘 · 点击生成新粒子")}</p>
        <div className="portal-hero-grid">
          <div className="portal-hero-copy">
            <p className="portal-kicker"><span>01</span> {L("AURORA ATLAS / 由意识编织")}</p>
            <h1><span>NEURAL</span><span>CONNECTION</span></h1>
            <p className="portal-lead">{L("一个由意识编织的")}<br />{L("数字宇宙")}</p>
            <nav className="portal-primary-paths" aria-label={L("咪呀空间入口")}>
              <Link to="/miia" className="portal-primary-path portal-primary-path--miia" data-motion-ripple="true">
                <Sparkles aria-hidden="true" /><span><small>MIIA / INNER SPACE</small><strong>{L("咪呀空间")}</strong><em>{L("从内心独白、数学遐想与诗歌碎片，进入咪呀的空间。")}</em></span><ArrowRight aria-hidden="true" />
              </Link>
            </nav>
          </div>
          <aside className="portal-miia">
            ...
            <div><strong>{L("积分结果 = 2e")}</strong><span>{L("你好～我是 ∫₀¹[...]d(2x) 的二年级生")}<br />{L("只想被你看见然后被爱")}</span></div>
          </aside>
      ...
      <section className="portal-section portal-activity-section" aria-labelledby="portal-activity-title">
        <div className="portal-section-head"><div><p>01 / FIRST CONNECTION</p><h2 id="portal-activity-title">{L("从这里继续")}</h2></div><span>{L("回到上次停下的地方，或从世界观、故事与角色开启新的旅程。")}</span></div>
        <div className="portal-activity-grid">
          <Link to={lastViewedTarget} className="portal-activity-card portal-activity-resume"><History /><span><small>CONTINUE / LAST VIEWED</small><strong>{L("上次在看")}</strong><em>{lastViewedDescription}</em></span><ArrowRight /></Link>
          <Link to="/world" className="portal-activity-card portal-activity-world"><Globe2 /><span><small>WORLD LORE / CANON</small><strong>{L("世界观")}</strong><em>{L("从地点、规则、组织和时间线，找到这座世界的坐标。")}</em></span><ArrowRight /></Link>
          <Link to="/stories" className="portal-activity-card portal-activity-story"><BookOpen /><span><small>STORY DIRECTORY / READ</small><strong>{L("故事")}</strong><em>{L("打开故事目录，选择一个宇宙和一段章节开始阅读。")}</em></span><ArrowRight /></Link>
          <Link to="/characters" className="portal-activity-card portal-activity-character"><Users /><span><small>CHARACTER NETWORK / MEET</small><strong>{L("角色")}</strong><em>{L("从人物档案、阵营和关系网，认识每一次相遇。")}</em></span><ArrowRight /></Link>
        </div>
      </section>

      <section className="portal-section portal-visions">
        <div className="portal-section-head"><div><p>02 / VISUAL ARCHIVE</p><h2>{L("世界的六个切面")}</h2></div><span>{L("在时间、角色与规则之间，图像是通往宇宙的第一道门。")}</span></div>
        <figure className="portal-overview">
          <ResponsiveImage
            src="/2-generated.png"
            alt={L("夕阳与星空下的星界馆，五个身影在环形图书馆中阅读、学习与交流")}
          ...
          <figcaption>
            <p>{L("UNIVERSE OVERVIEW / 星界馆")}</p>
            <h3>{L("星界馆 · 宇宙总览")}</h3>
            <span>{L("意识、时间与城市在同一座馆室交汇，六个切面由此展开。")}</span>
          </figcaption>
        </figure>
        ...
        <div
          ...
          role="region"
          aria-roledescription={_en ? 'carousel' : '轮播图'}
          aria-label={L("世界的六个切面视觉档案")}
        ...
        >
          <div
            ...
            role="group"
            aria-roledescription={_en ? 'slide' : '幻灯片'}
            aria-label={L(`第 ${slide + 1} 张，共 ${slides.length} 张：${slides[slide].title}`)}
          >
            <ResponsiveImage
            ...
            <button type="button" className="portal-arrow portal-prev" data-motion-click="none" onClick={() => go(slide - 1)} aria-controls="portal-archive-slide" aria-label={L(`上一张：${slides[(slide - 1 + slides.length) % slides.length].title}`)}><ArrowLeft aria-hidden="true" /></button>
            <button type="button" className="portal-arrow portal-next" data-motion-click="none" onClick={() => go(slide + 1)} aria-controls="portal-archive-slide" aria-label={L(`下一张：${slides[(slide + 1) % slides.length].title}`)}><ArrowRight aria-hidden="true" /></button>
          </div>
          <div className="portal-thumbs" role="group" aria-label={L("选择视觉档案")}>{slides.map((item, index) => <button key={item.src} type="button" className={index === slide ? 'active' : ''} onClick={() => go(index)} aria-label={L(`查看第 ${index + 1} 张：${item.alt}`)} aria-controls="portal-archive-slide" aria-pressed={index === slide} aria-current={index === slide ? 'true' : undefined}><ResponsiveImage src={item.src} alt="" widths={THUMBNAIL_IMAGE_WIDTHS} sizes="(max-width: 640px) 30vw, 180px" loading="lazy" /><span>{String(index + 1).padStart(2, '0')}</span></button>)}</div>
          <span className="portal-carousel-status" aria-live="polite">{carouselPaused ? (_en ? 'Auto-play paused' : '自动轮播已暂停') : (_en ? 'Auto-play playing' : '自动轮播播放中')}</span>
        </div>
      </section>

      <section className="portal-section portal-worlds" aria-labelledby="portal-directory-title">
        <div className="portal-section-head"><div><p>03 / COMPLETE DIRECTORY</p><h2 id="portal-directory-title">{L("完整目录")}</h2></div><span>{L("从故事、角色和世界观，到数学、游戏与对话，所有内容入口都汇集在这里。")}</span></div>
        <div className="portal-entry-grid portal-system-grid">
          {NAVIGATION_GROUPS.map((group, index) => {
            const Icon = directoryIconMap[group.id];
            const en = NAVIGATION_GROUP_EN[group.id];
            const caption = _en && en ? en.caption : group.caption;
            const label = _en && en ? en.label : group.label;
            const description = _en && en ? en.description : group.description;
            return <Link to={group.root} key={group.id} className={`portal-entry portal-entry-${group.id}`} style={{ '--card': directoryAccentMap[group.id] } as React.CSSProperties}>
              <div className="portal-entry-top"><span>{String(index + 1).padStart(2, '0')}</span><span>{caption}</span></div>
              <div className="portal-entry-title"><Icon /><div><p>SPACE / {caption}</p><h3>{label}</h3></div></div>
              <p>{description}</p>
              <div className="portal-entry-link"><span>{L("进入")}{label}</span><span>↗</span></div>
            </Link>;
          })}
        </div>
      </section>
      ...
      <section className="portal-section portal-support-section" aria-labelledby="portal-support-title">
        <div className="portal-section-head"><div><p>04 / SITE TOOL</p><h2 id="portal-support-title">{L("需要时，问一次就好")}</h2></div><span>{L("输入想找的人物、故事或设定，AI 会带着站内来源一起回答。")}</span></div>
        <div className="portal-support-layout portal-support-layout--single">
          <NeuralCommandConsole />
        </div>
      </section>

      <section className="portal-closing"><p>{L("“世界不是一座孤岛。”")}</p><span>{L("角色、世界与游戏，沿着神经连接彼此感应，让每一次相遇成为新的回响。")}</span></section>
    </div>
  );
}
