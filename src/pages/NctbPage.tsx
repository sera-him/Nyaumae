...
type PageScreen = 'dashboard' | 'session' | 'history';
    ...
    <>
      <header className="nctb-hero">
        <div className="nctb-hero-copy">
          ...
          <h1>认知实验室</h1>
          <p className="nctb-lead">十个维度，五十道题，一套可中断、可恢复、可解释的本地能力探索闭环。</p>
          <div className="nctb-hero-actions">
            <button type="button" className="nctb-primary-action" onClick={() => onCreate('formal')}><ShieldCheck size={17} />开始考试<ArrowRight size={16} /></button>
            <button type="button" className="nctb-secondary-action" onClick={onHistory}><History size={16} />历史与趋势</button>
          </div>
        </div>
        <div className="nctb-hero-orbit" aria-hidden="true">
          ...
          <div className="nctb-orbit-core"><BrainCircuit size={34} /><span>EXPLORATION</span><strong>{latestReport?.composite?.score ?? '—'}</strong><small>{latestReport?.composite ? '综合探索分' : '等待首份报告'}</small></div>
        </div>
      </header>

      <section className="nctb-control-bar" aria-label="实验室状态">
        <div><span>本机报告</span><strong>{state.reports.length}</strong></div>
        <div><span>可恢复会话</span><strong>{resumable.length}</strong></div>
        <div><span>最近活动</span><strong>{latestReport ? formatDate(latestReport.createdAt) : '尚无'}</strong></div>
        <div className="nctb-control-note"><ShieldCheck size={17} /><span>仅保存在本机<br /><b>不用于医疗或教育诊断</b></span></div>
      </section>

      {state.legacy && (
        <section className="nctb-legacy-note" role="status">
          <History size={18} />
          <div><strong>旧版进度仍被保留</strong><p>检测到 v{state.legacy.sourceVersion ?? 2} 的 {state.legacy.answeredCount} 道作答、{state.legacy.completedParts} 个已完成部分。新版不会把旧选项误算成新报告；旧数据会一直保留到你主动重置。</p></div>
        </section>
      )}

      {resumable.length > 0 && (
        <section className="nctb-resume-section" aria-labelledby="nctb-resume-title">
          <div className="nctb-section-heading"><div><p className="nctb-eyebrow">RESUMABLE SESSIONS</p><h2 id="nctb-resume-title">从上次停下的位置继续</h2></div><span>题目顺序、选项顺序、当前难度与有效作答都由同一个 seed 恢复。</span></div>
          <div className="nctb-session-list">
            {resumable.map((session) => {
              ...
              return <button type="button" key={session.id} onClick={() => onResume(session)} className="nctb-session-card">
                ...
                <span><small>{MODE_LABELS[session.mode]} · {STRATEGY_LABELS[session.strategy]}</small><strong>{session.focusDimension === 'all' ? '十维完整考试' : `${getNctbDimension(session.focusDimension).title}专项考试`}</strong><i><b style={{ width: `${completion.percentage}%` }} /></i></span>
      ...
      <section className="nctb-dimensions-section" aria-labelledby="nctb-dimensions-title">
        <div className="nctb-section-heading"><div><p className="nctb-eyebrow">THE TEN DIMENSIONS</p><h2 id="nctb-dimensions-title">动态能力图谱</h2></div><span>每一维都同时呈现正确率、有效作答时间、难度和 95% 置信区间，不压缩成 IQ。</span></div>
        <div className="nctb-parts-grid">
          {NCTB_DIMENSIONS.map((dimension) => {
      ...
      <section className="nctb-safety-note"><ShieldCheck size={20} /><div><strong>数据与解释边界</strong><p>标准考试只是本地一致化操作，不是受监管或保密的心理测验。综合探索分是十维描述性分数的平均，其中速度维同时考虑正确率与目标用时，只用于回顾个人表现。</p></div><button type="button" onClick={onReset}><Trash2 size={13} />重置全部本机数据</button></section>
    </>
  );
}
...
function SetupView({ session, onChange, onStart, onCancel }: {
  ...
  return (
    <section className="nctb-setup" aria-labelledby="nctb-setup-title">
      ...
      <div className="nctb-workspace-head"><div><p className="nctb-eyebrow">SESSION SETUP</p><h2 id="nctb-setup-title">建立一次可复现的探索会话</h2><p>开始后会锁定模式、题库版本、随机 seed 与难度策略。</p></div><span className="nctb-mode-badge">SEED / {session.seed.toString(16).toUpperCase()}</span></div>

      <div className="nctb-setup-grid">
        <fieldset className="nctb-setup-card"><legend>01 · 考试模式</legend>
          <button type="button" className="is-active" disabled><span><ShieldCheck size={18} /></span><div><strong>标准考试</strong><p>全部题目统一作为考试题；整套完成后统一反馈，不显示提示、解释或单题对错。</p></div><Check size={16} /></button>
        </fieldset>
        <fieldset className="nctb-setup-card"><legend>02 · 选题策略</legend>
          {(['fixed', 'adaptive'] as const).map((strategy) => <button type="button" key={strategy} disabled={strategy === 'adaptive'} className={session.strategy === strategy ? 'is-active' : ''} onClick={() => onChange({ strategy })}><span>{strategy === 'fixed' ? <Target size={18} /> : <Zap size={18} />}</span><div><strong>{STRATEGY_LABELS[strategy]}</strong><p>{strategy === 'fixed' ? '题库按目标难度优先排序，并完整记录实际难度。' : '题量扩充并完成标定后开放；当前题库不伪装成有效自适应测试。'}</p></div><Check size={16} /></button>)}
        </fieldset>
      </div>
      ...
      <div className="nctb-setup-summary"><div><span>预计作答</span><strong>{required} 题</strong></div><div><span>维度覆盖</span><strong>{session.dimensionOrder.length} / 10</strong></div><div><span>题库版本</span><strong>v{session.bankVersion}</strong></div><button type="button" className="nctb-primary-action" onClick={onStart}><Play size={16} />锁定设置并开始<ArrowRight size={15} /></button></div>
    </section>
  );
}
...
function ActiveRunner({
  ...
  const options = orderedQuestionOptions(session);
  ...
  if (!question) return <div className="nctb-empty-state"><CircleHelp size={28} /><h2>无法恢复当前题目</h2><p>题库版本与会话不匹配。请返回实验室新建会话，旧作答仍保留在本机。</p></div>;
  ...
  return (
    <section className="nctb-runner" aria-labelledby="nctb-question-title">
      <header className="nctb-runner-header">
        ...
        <div className="nctb-runner-metrics"><span><Clock3 size={14} />有效用时 <b>{formatDuration(session.activeMs)}</b></span><span><Gauge size={14} />难度 <b>{question.difficulty} / 5</b></span><button type="button" onClick={onPause}><Pause size={14} />暂停</button></div>
      </header>
      <div className="nctb-progress-panel"><div><span>总进度 {completion.answered} / {completion.required}</span><b>{completion.percentage}%</b></div><div className="nctb-progress-track" role="progressbar" aria-label="整套作答进度" aria-valuemin={0} aria-valuemax={100} aria-valuenow={completion.percentage}><i style={{ width: `${completion.percentage}%` }} /></div></div>

      <div className="nctb-question-card">
        <div className="nctb-question-meta"><span>QUESTION {String(questionNumber).padStart(2, '0')} / {required}</span><span>{MODE_LABELS[session.mode]} · {STRATEGY_LABELS[session.strategy]}</span></div>
        {question.kind === 'memory' && !session.currentStimulusSeen && <div className="nctb-memory-ready"><BrainCircuit size={28} /><h3>准备查看记忆序列</h3><p>点击后序列只显示约 {((question.revealMs ?? 3_500) / 1_000).toFixed(1)} 秒。切到后台不会继续累计有效用时。</p><button type="button" className="nctb-primary-action" onClick={revealStimulus}><Play size={15} />显示序列</button></div>}
        {question.kind === 'memory' && memoryShowing && <div className="nctb-memory-stimulus" role="timer"><span>请记住</span><strong>{question.stimulus}</strong><small>{Math.max(0, ((revealUntil ?? 0) - revealNow) / 1_000).toFixed(1)} 秒</small></div>}
          {memoryReady && <>
           ...
           {response && <div className="nctb-answer-feedback is-formal" role="status"><ShieldCheck size={17} /><span><strong>答案已锁定</strong>单题对错将在整套考试结束后汇总。</span></div>}
           <footer className="nctb-challenge-footer"><span>本题有效用时 {formatDuration(session.currentItemActiveMs)} · 目标参考 {formatDuration(question.targetMs)}</span><div>{response && <button type="button" className="is-primary" onClick={onAdvance}>{answeredInDimension >= required ? '完成本部分' : '下一题'}<ArrowRight size={13} /></button>}</div></footer>
        </>}
      </div>
      <p className="nctb-keyboard-note">键盘提示：题目出现后可按 1–4 选择答案。考试模式不显示提示，也不允许重试。</p>
    </section>
  );
}
...
function PausedView({ session, onResume, onDashboard }: { session: NctbSession; onResume: () => void; onDashboard: () => void }) {
  ...
  const copy = session.pauseReason === 'section-break'
    ? { title: '上一部分已完成', detail: `下一部分是“${nextDimension.title}”。休息不会累计有效用时。`, icon: <Check size={28} /> }
    : session.pauseReason === 'friendly-break'
      ? { title: '友好停靠点', detail: '已经连续完成三题。可以现在继续，也可以稍后从同一个 seed 恢复。', icon: <Sparkles size={28} /> }
      : { title: '会话已暂停', detail: '页面后台时间和暂停时间不会计入 activeMs。', icon: <Pause size={28} /> };
  return <section className="nctb-pause-view"><div className="nctb-pause-icon">{copy.icon}</div><p className="nctb-eyebrow">PAUSED / LOCAL CHECKPOINT</p><h2>{copy.title}</h2><p>{copy.detail}</p><div><button type="button" className="nctb-primary-action" onClick={onResume}><Play size={15} />继续考试</button><button type="button" className="nctb-secondary-action" onClick={onDashboard}>返回实验室</button></div></section>;
}
...
function CompletedView({ session, report, persisted, onOpen, onDashboard }: { session: NctbSession; report?: NctbReport; persisted: boolean; onOpen: () => void; onDashboard: () => void }) {
  return <section className="nctb-completed-view"><div className="nctb-complete-mark"><Check size={34} /></div><p className="nctb-eyebrow">EXAM COMPLETED</p><h2>{persisted ? '本次考试已经完整保存' : '本次考试已完成，但未能写入本机存储'}</h2><p>{persisted ? `共记录 ${report?.totalAnswered ?? effectiveResponses(session).length} 道有效作答，计时、难度、置信区间和考试建议已写入本机报告。` : `当前页面仍保留 ${report?.totalAnswered ?? effectiveResponses(session).length} 道有效作答；请立即导出报告，关闭或刷新页面后这些数据可能丢失。`}</p><div><button type="button" className="nctb-primary-action" onClick={onOpen} disabled={!report}><BarChart3 size={16} />查看完整报告<ArrowRight size={15} /></button><button type="button" className="nctb-secondary-action" onClick={onDashboard}>返回实验室</button></div></section>;
}
...
function ReportView({
  ...
  const radarData = NCTB_DIMENSIONS.map((dimension) => ({
  ...
  return (
    <section className="nctb-report" aria-labelledby="nctb-report-title">
      <div className="nctb-report-toolbar"><button type="button" className="nctb-back-button" onClick={onBack}><ArrowLeft size={15} />返回</button><div><button type="button" onClick={exportJson}><FileJson size={14} />JSON</button><button type="button" onClick={exportCsv}><Download size={14} />CSV</button><button type="button" onClick={() => window.print()}><Printer size={14} />打印 / PDF</button></div></div>
      <header className="nctb-report-hero"><div><p className="nctb-eyebrow">LOCAL EXPLORATION REPORT</p><h2 id="nctb-report-title">NCTB 本地探索报告</h2><p>{formatDate(report.createdAt)} · {MODE_LABELS[report.mode]} · {STRATEGY_LABELS[report.strategy]}{report.friendlyMode ? ' · 启动友好模式' : ''}</p></div><div className="nctb-report-score"><span>{report.composite ? '综合探索分' : '专项报告'}</span><strong>{report.composite?.score ?? report.dimensions[0]?.score ?? '—'}</strong><small>{report.composite ? `${report.composite.coverage} 个维度的描述性汇总` : `${report.dimensions.length} 个维度有数据`}</small></div></header>
      <div className="nctb-report-stat-grid"><div><span>有效作答</span><strong>{report.totalAnswered}</strong></div><div><span>正确</span><strong>{report.totalCorrect}</strong></div><div><span>有效用时</span><strong>{formatDuration(report.totalActiveMs)}</strong></div><div><span>中断次数</span><strong>{report.interruptionCount}</strong></div></div>

      <div className="nctb-report-grid">
        <article className="nctb-chart-card"><header><div><p className="nctb-eyebrow">TEN-DIMENSION PROFILE</p><h3>十维能力轮廓</h3></div><span>0–100 描述性分数</span></header><div className="nctb-radar-chart" aria-label="十维能力雷达图"><ResponsiveContainer width="100%" height="100%"><RadarChart data={radarData} outerRadius="72%"><PolarGrid stroke="rgba(255,255,255,.15)" /><PolarAngleAxis dataKey="dimension" tick={{ fill: '#b8cecc', fontSize: 11 }} /><Radar dataKey="score" stroke="#74efe0" fill="#74efe0" fillOpacity={0.24} strokeWidth={2} /><Tooltip contentStyle={{ background: '#0c1820', border: '1px solid rgba(116,239,224,.25)', borderRadius: 12 }} /></RadarChart></ResponsiveContainer></div><p>雷达图只是视觉摘要；下方每一维的样本量与区间更重要。</p></article>
        <article className="nctb-recommendation-card"><header><p className="nctb-eyebrow">NEXT EXAM</p><h3>下一次考试建议</h3></header><p>所有题目都属于标准考试题。下一次完整考试仍会覆盖十个维度和全部 50 道题，下面的维度可作为回顾重点。</p><div className="nctb-recommendation-list">{report.focusAreas.map((dimensionId) => { const dimension = getNctbDimension(dimensionId); return <div key={dimensionId}><span style={{ background: dimension.accent }} /><div><strong>{dimension.title}</strong><small>下一次考试继续观察</small></div></div>; })}</div><button type="button" className="nctb-retest-button" onClick={onRetest}><RefreshCw size={14} />重新开始考试</button></article>
      </div>

      <section className="nctb-result-list" aria-labelledby="nctb-result-list-title"><div className="nctb-section-heading"><div><p className="nctb-eyebrow">DIMENSION DETAILS</p><h2 id="nctb-result-list-title">每一维的证据</h2></div><span>Wilson 区间只描述正确率的不确定性，不是综合分的置信区间；不同设置下的结果不应当作同一标准量表比较。</span></div><div>{report.dimensions.map((result) => { const dimension = getNctbDimension(result.dimensionId); return <article key={result.dimensionId} style={{ '--dimension-accent': dimension.accent } as React.CSSProperties}><header><span>{dimension.index}</span><div><strong>{dimension.title}</strong><small>{dimension.english}</small></div><b>{result.score}</b></header><div className="nctb-result-track"><i style={{ width: `${result.score}%` }} /><span style={{ left: `${result.ciLow}%`, width: `${Math.max(1, result.ciHigh - result.ciLow)}%` }} /></div><footer><span>{result.correct} / {result.answered} 正确 · {result.accuracy}%</span><span>正确率 95% CI {result.ciLow}–{result.ciHigh}</span><span>中位用时 {formatDuration(result.medianActiveMs)}</span><span>{result.scoreModel === 'speed-accuracy' ? '分数含 25% 目标用时权重' : `平均难度 ${result.averageDifficulty}`}</span></footer></article>; })}</div></section>
      <aside className="nctb-report-disclaimer"><ShieldCheck size={19} /><p><strong>解释边界：</strong>综合探索分是十维描述性分数的平均；速度维含 25% 目标用时权重，其余维度使用平滑正确率。它不是 FSIQ，尚未经过心理测量标定，也没有调用角色世界观使用的 FSIII 公式。</p></aside>
    </section>
  );
}
...
function HistoryView({ reports, onBack, onOpen }: { reports: NctbReport[]; onBack: () => void; onOpen: (report: NctbReport) => void }) {
  ...
  return <section className="nctb-history" aria-labelledby="nctb-history-title"><button type="button" className="nctb-back-button" onClick={onBack}><ArrowLeft size={15} />返回实验室</button><div className="nctb-section-heading"><div><p className="nctb-eyebrow">LOCAL HISTORY</p><h2 id="nctb-history-title">历史报告与趋势</h2></div><span>趋势只比较同一维度；模式、策略和友好设置会保留在每份报告上。</span></div>
    {reports.length === 0 ? <div className="nctb-empty-state"><History size={28} /><h3>还没有历史报告</h3><p>完成一次标准考试后，趋势会出现在这里。</p></div> : <>
      <article className="nctb-trend-card"><header><div><p className="nctb-eyebrow">SCORE TREND</p><h3>{getNctbDimension(dimensionId).title}趋势</h3></div><label>选择维度<select value={dimensionId} onChange={(event) => setDimensionId(event.target.value as NctbDimensionId)}>{NCTB_DIMENSIONS.map((dimension) => <option value={dimension.id} key={dimension.id}>{dimension.title}</option>)}</select></label></header><div><ResponsiveContainer width="100%" height="100%"><LineChart data={trendData} margin={{ top: 15, right: 20, left: -15, bottom: 0 }}><CartesianGrid stroke="rgba(255,255,255,.08)" strokeDasharray="4 4" /><XAxis dataKey="date" tick={{ fill: '#92acab', fontSize: 11 }} /><YAxis domain={[0, 100]} tick={{ fill: '#92acab', fontSize: 11 }} /><Tooltip contentStyle={{ background: '#0c1820', border: '1px solid rgba(116,239,224,.25)', borderRadius: 12 }} /><Line type="monotone" dataKey="score" name="探索分" stroke="#74efe0" strokeWidth={2.5} dot={{ r: 4, fill: '#74efe0' }} /></LineChart></ResponsiveContainer></div>{trendData.length < 2 && <p>需要至少两份包含该维度的报告才能形成趋势线。</p>}</article>
      <div className="nctb-history-list">{[...reports].sort((left, right) => right.createdAt.localeCompare(left.createdAt)).map((report) => <button type="button" key={report.id} onClick={() => onOpen(report)}><span className="nctb-history-score">{report.composite?.score ?? report.dimensions[0]?.score ?? '—'}</span><span><small>{MODE_LABELS[report.mode]} · {STRATEGY_LABELS[report.strategy]}{report.friendlyMode ? ' · 友好' : ''}</small><strong>{report.focusDimension === 'all' ? '十维探索报告' : `${getNctbDimension(report.focusDimension).title}专项报告`}</strong><em>{formatDate(report.createdAt)} · {report.totalAnswered} 题 · {formatDuration(report.totalActiveMs)}</em></span><ChevronRight size={17} /></button>)}</div>
    </>}
  </section>;
}

function ResetDialog({ onCancel, onConfirm }: { onCancel: () => void; onConfirm: () => void }) {
  return <div className="nctb-dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel(); }}><section className="nctb-dialog" role="alertdialog" aria-modal="true" aria-labelledby="nctb-reset-title" aria-describedby="nctb-reset-description"><button type="button" className="nctb-dialog-close" onClick={onCancel} aria-label="关闭"><X size={16} /></button><Trash2 size={24} /><h2 id="nctb-reset-title">重置全部本机 NCTB 数据？</h2><p id="nctb-reset-description">这会删除新版会话、报告、趋势和保留的旧版 v2 进度。导出的 JSON / CSV 文件不会受影响。</p><div><button type="button" onClick={onCancel}>取消</button><button type="button" className="is-danger" onClick={onConfirm}>确认删除</button></div></section></div>;
}
  ...
  return (
    <div className="nctb-page aurora-ui" data-aurora-accent="math">
      <div className="nctb-shell">
        {screen === 'dashboard' && <Dashboard state={store} onCreate={createSession} onResume={resumeSession} onHistory={() => setScreen('history')} onReset={() => setShowReset(true)} />}
        {screen === 'history' && <HistoryView reports={store.reports} onBack={backToDashboard} onOpen={openHistoryReport} />}
        {screen === 'session' && (sessionContent ?? <div className="nctb-empty-state"><CircleHelp size={28} /><h2>没有可显示的会话</h2><button type="button" className="nctb-primary-action" onClick={backToDashboard}>返回实验室</button></div>)}
        {screen !== 'session' && <footer className="nctb-page-footer"><span>NCTB 数据不会自动发送到服务器。</span><Link to="/chat/ocean?mode=nctb-proctor&prompt=NCTB%E6%B5%81%E7%A8%8B%E8%AF%B4%E6%98%8E%EF%BC%9A%E8%AF%B7%E5%8F%AA%E8%A7%A3%E9%87%8A%E6%93%8D%E4%BD%9C%E3%80%81%E6%9A%82%E5%81%9C%E5%92%8C%E9%9A%90%E7%A7%81%E8%A7%84%E5%88%99%EF%BC%8C%E4%B8%8D%E8%A6%81%E6%8F%90%E7%A4%BA%E7%AD%94%E6%A1%88%E3%80%82"><BrainCircuit size={14} />在星海对话查看流程说明<ArrowRight size={13} /></Link></footer>}
      </div>
